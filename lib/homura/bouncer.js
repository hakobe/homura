...
function Bouncer(options) {
    ...
    this.ircClientRetryCount = 10;
    this.ircClientRetryDelay = 30 * 1000;
...
Bouncer.prototype.connect = function() {
    Log.debug(this.tagForLog(), 'Starting connection');
    if (!this.ircClient) {
        ...
        this.ircClient.on('close', (function(message) {
            Log.debug(this.tagForLog(), 'Closing connection');
            this.ircClient = null;
            setTimeout( (function() {
                if ( this.ircClientRetryCount > 0 ) {
                    Log.info(this.tagForLog(), 'Try reconnecting. RetryCount = ' + this.ircClientRetryCount);
